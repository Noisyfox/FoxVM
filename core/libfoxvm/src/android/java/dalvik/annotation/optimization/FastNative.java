...
/**
 * FoxVM-changed: This has different meaning in FoxVM. I reuse the same class as Android ART,
 * however in foxvm this indicates the translator not to generate the JNI bridge method for annotated
 * native method. Instead, a minimum fast-native bridge method is generated that calls another manually written
 * method immediately, with the original method info passed in as the second parameter. The second method will
 * be implemented manually use the same way as other translated non-native method, to avoid the state transaction
 * when making the normal JNI calls.
 */
